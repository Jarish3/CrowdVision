...
//GET tempo attesa per info page di singola mensa
app.get('/tempo_attesa', function(request, response, next) {
	pg.connect(connectionString, function(err, client, done) {
		if (err)
			return console.error('Errore durante il recupero del client dal pool', err);
		...
		//Struttura request: params??? <---------------------------------------------------
		client.query('SELECT M.Id_mensa, M.Nome_mensa, M.Tempo_sevizio, F.N_persone FROM MENSE M INNER JOIN FOTO F ON M.Id_mensa=F.Id_mensa AND F.Data = (SELECT MIN(Data) FROM FOTO F WHERE F.Id_mensa=$1)', [request.params.id], function(err, result) {
			done();
			if (err)
				return console.error('Errore durante esecuzione della query', err);
